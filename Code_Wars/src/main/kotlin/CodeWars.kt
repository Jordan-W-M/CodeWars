...
//        if (input != null)arrayOf(input.count { it >= 1 },input.filter {it <= -1}.sum()) else emptyArray()
    ...
    /**
     * Create a function that compares the cost of 2 methods till method b is cheaper to the nearest whole number
     * The first method has a set number for the purchase of a ticket
     * The second method has large upfront payment that follows by ever degreesing smaller payments for further tickets
     *
     * The function need to take both the values of a and b
     * (largePayment:Int , ticketPrice: Int , percentageOffTicketForLargePayment: Double
     */

    fun movie(card:Int, ticket:Int, perc:Double): ArrayList<Double> {
        var counter = 0
        var cardCounter:Double = card.toDouble()
        var ticketCounter = ticket
        var percentageCounter:Double = ticket.toDouble()
        var tester = arrayListOf<Double>()
        while (cardCounter.roundToInt() >= ticketCounter ){
            ticketCounter += ticket
            if (cardCounter != card.toDouble())percentageCounter *= perc
            cardCounter += if (cardCounter == card.toDouble()){
                ticket.toDouble()
            }else{
                percentageCounter
            }
            tester.add(cardCounter
            )
            counter++
        }
        return tester
    ...
    fun movie2(card:Int, ticket:Int, perc:Double): Int {
        var counter = 0
        var cardCounter:Double = card.toDouble()
        var ticketCounter = 0
        var percentageCounter:Double = ticket.toDouble()
        while (cardCounter.roundToInt() >= ticketCounter || cardCounter >= ticketCounter ){
            ticketCounter += ticket
            percentageCounter *= perc
            cardCounter += percentageCounter
            counter++
        }
        return counter
    }

    fun movie3(card:Int, ticket:Int, perc:Double): Int {
        var counter = 0
        var cardCounter:Double = card.toDouble()
        var percentageCounter:Double = ticket.toDouble()
        var ticketCounter = 0
        while (cardCounter >= (ticket * counter) - 0.5){
            counter++
            ticketCounter += ticket
            percentageCounter *= perc
            cardCounter += percentageCounter
        ...
        return counter
    }// The was my answer, but it could be a lot better
    ...
    fun seven(n:Long): ArrayList<Long> {
        var mValue:Long = n
        var yValue = mValue.toString().last().toString().toLong() * 2
        var xValue = mValue.toString().dropLast(1).toLong()
        val returnValue = arrayListOf(mValue , 0)
        var tester = arrayListOf<Long>()
        while (n == n){
            mValue = (xValue - yValue).toLong()
            returnValue[0] = mValue
            returnValue[1] = returnValue[1].plus(1)
            tester.add(mValue)
            if (xValue in 0..100){ break }else {
                xValue = mValue.toString().dropLast(1).toLong()
                yValue = mValue.toString().last().toString().toLong() * 2
            }
        }
        // My answer
        // It works, but it's a very inefficient way of doing things
        // I need to put more focus on breaking things down into simple math equations that solve the answer
        return returnValue
    }
    ...
    val randomBoi = (100 until 90000000).random()
    println(seven3(randomBoi.toLong()))
    println(seven2(randomBoi.toLong()))
    println(seven(randomBoi.toLong()))

    /**
     * Create a function that takes a string of Ints and returns the highest and lowest value Int
     */

//    val tester ="8 3 -5 42 -1 0 0 -9 4 7 4 -4"
//    println(tester.split(" "))
//    val tester2 = tester.split(" ")
//    var tester3 = arrayListOf<Int>()
//    for (numbs in tester2){
//        tester3.add(numbs.toInt())
//    }
//    tester3.sort()
//    println(tester3[0])
